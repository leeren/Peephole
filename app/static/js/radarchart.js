var RadarChart = {
  draw: function(id, d, options){
    ...
    function drawnode(){   
      g.selectAll(".nodes")
        ...
        .on('mouseover', function (d){
                    ...
                    tooltip.attr('x', newX).attr('y', newY).text(d3.round(d.value)).transition(200).style('opacity', 1);
                    z = "polygon."+d3.select(this).attr("class");
                    g.selectAll("polygon").transition(200).style("fill-opacity", 0.1);
                    g.selectAll(z).transition(200).style("fill-opacity", 0.7);
                  })
    ...
    function move(dobj, i){
      ...
      // 進行座標位移歸零，以方便計算斜率: Perform coordinate zeroing to facilitate the calculation of slope
      ...
      // 斜率為無限大的特殊情況: A special case where the slope is infinite
      if(oldX == 0) {
        newY = oldY - d3.event.dy;
        // 檢查是否超過範圍: Check if the range is exceeded
      ...
      else
      {
        ...
        var ratio = newX / oldX; // 利用相似三角形等比的概念計算新的值: The new values are calculated using the concept of similar triangles
        newValue = ratio * oldData.value;
      }

      // 重新設定點的座標: Resets the coordinates of the point
      dragTarget
          .attr("cx", function(){return newX + cfg.w/2 ;})
          .attr("cy", function(){return cfg.w/2 - newY;});
      // 重新設定點的值: Resets the value of the point
      d[oldData.order].value=newValue;
      // 重新計算多邊形的轉折點: Recomputes the inflection point of the polygon
      reCalculatePoints();
      // 重畫多邊形: Redraws the polygon
      drawPoly();
    }
